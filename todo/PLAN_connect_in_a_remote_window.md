...
- [ ] A WSL window with a stored key and a running relay connects, and the key never enters the
      distribution.
- [ ] A WSL window without a relay refuses, names every missing piece at once, and offers the button
      that fixes it — it never emits a command that cannot work.
- [ ] Every other remote window kind refuses with a message naming which machine holds the key.
- [ ] A local window's behaviour is byte-identical to today, asserted by the existing suite.
...
- [ ] `npm test` green in `src_vs_code`; the red test above was watched failing first and its message
      reported.
- [ ] `scripts/wsl-agent-relay-itest.cjs` **actually run on a machine with WSL** and its output
      quoted in the summary — not merely skipped green.
- [ ] `research/module_extension.md` and `research/module_tests.md` updated; the boundary table
      written into `PLAN_wsl_agent_relay.md`, `PLAN_remote_broker_bridge.md` and `PLAN_tails_2.md`;
      this plan promoted with its deviations recorded.
