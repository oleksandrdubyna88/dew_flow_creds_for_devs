...
| [PLAN_headless_cli.md](PLAN_headless_cli.md) | plan only | `creds ssh prod-db` from any terminal. ~75% exists (`agentCli.ts` already talks to the broker headlessly); owner decision: a standalone **Rust/AOT binary**, not a node wrapper — so the wire contract becomes a generated JSON spec with two implementations. Also carries a **live bug fix**: `env`/`vpn-up`/`vpn-down` report success as exit 95 |
...
| [PLAN_remote_broker_bridge.md](PLAN_remote_broker_bridge.md) | plan only — **needs an owner answer before phase 2** | Broker reachable from Remote-SSH / Dev Container / WSL, consent on the local machine, no secret on the remote disk. Largest of the five (~10-15% exists): the broker is deliberately loopback-TCP-only and nothing implements the ssh-agent protocol. Phase 1 (unix socket) is useful on its own; phase 3 depends on the frozen D1 |
