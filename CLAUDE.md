...
## Review gate (ConnectOtherAIs)

Non-trivial work here goes through the `coai` gate before and after implementation. **The rule is
shared, not pasted**: [.claude/rules/shared/common/coai-review-gate.md](.claude/rules/shared/common/coai-review-gate.md),
loaded in every session from the `dew_flow_conventions` submodule like every other family rule.

It used to be a block of text pasted into this file, and on 2026-09-04 that paste was found to be
three revisions behind what the extension hands out — missing the operator COMMANDS block, the
"reject in round 1" rule and the enforced stop after `call_human`. Nobody was careless; that is what
happens to a copy. `node .claude/rules/shared/tools/gate-snippet-check.mjs` now fails the build if a
copy reappears here.
