...
| The agent-doors footer (T24b) | `agentDoors.ts` (`agentDoorRows`, `doorsOf`, `standingConsentFor`), `agentDoorsHtml()` in `entityFormPage.ts`, `runDoorCommand` in `entityFormHost.ts` | Doors have one owner each — the form lists the live ones and links to the owning command; nothing live, no footer. **A never-ask entry is one of those doors** (#95, S3.3) and is listed first, with the modal-free ones: `standingConsent` is `ask === 'never'` AND `use === true`, both halves, because the row says an agent may USE this entry without being asked and with `use` off no agent can use it at all. Its wording says *saved as*, since it is the one row whose value the form itself can change. The CLI row's claim that the alias routes have **no consent modal was false** and is corrected in the same change: `handleAlias` mints a grant and calls `perform`, which calls `consent` — an alias call raises the same dialog a token call does. The code-access key's *no consent modal* is true and stays |
