...
`envCollectionRef.ts` rather than `envApply.ts` on purpose: `envApply.ts` names `vscode` only in type
positions, so the import is elided and the module is pure at run time — `wovenPasswordForm.test.ts`
imports `automaticRefusal` from it with no stub, and one `vscode.window` call there failed that file at
load. The agent's `exportEnv` verb keeps answering `{ written }` — `EnvExportResponseBody` is a broker
contract the CLI and the MCP read — so a binding the policy withheld is not in that answer; widening
it is a contract change across three codebases and is recorded here as the open tail. The form's row
...
there it mattered more, because until the timeout below that prompt never expired. Its clock is a constructor argument
...
ask* — governs `/v1/mcp/use/*` and nothing else, because `preConsent` is wired into `mcpDoor` and
into no other route. The two doors it does not reach already answer the question themselves, in
opposite directions, which is why neither needed a cadence. On the **token** door the grant is the
token rather than the call — `grantForToken` finds the same grant for every request carrying that
Authorization header — so one Allow already covers every later call for the token's life, and a
window measured in hours could only make a wider grant look narrower than it is. On the **alias**
route `handleAlias` mints a *fresh* grant per call and hands it to `perform`, so nothing has marked
it allowed and `consent` asks — every time, for every entry, whatever its policy says. A never-ask
entry is therefore still asked on `creds ssh <alias>`, which is exactly what the doors footer's CLI
row now says, sitting as it does under a row telling the same person an agent may use this entry
without being asked.
...
| **Allow once / Allow for 10 minutes / Deny** | A `git push` signs and authenticates in one breath; a strictly-per-use modal would have taught people to click through it. The window is per key, in memory, and stated in the dialog. The ten minutes start at the click, not at the request — `SshAgentManager` reads one `clock` for both, and the prompt says when it was asked (*Requested … (UTC±HH:MM).*, #131). Unanswered, the prompt refuses after `CONSENT_TIMEOUT_MS` (five minutes, `agentConsent.ts` — one constant for this prompt and the broker's modal) through `withTimeout`; a late click lands on a settled promise and signs nothing. `ssh-agent-itest.cjs` drives that path with the real manager and the real `ssh-keygen` ([PLAN_ssh_sign_prompt_timeout.md](PLAN_ssh_sign_prompt_timeout.md)) |
...
in `mcpConsentPolicy.ts`, not beside the hooks, because a Forget that built its own `ConsentStamps`
would queue against a different queue from the one that writes stamps. Two `ConsentStamps` over one
`Memento` are two `SerialQueue`s, and the queue is what stops one window's concurrent writes
composing onto the map they both read before either landed; the key is the store's **identity**,
which is why a structural interface is enough. Across WINDOWS there is no such lock:
...
| The agent-doors footer (T24b) | `agentDoors.ts` (`agentDoorRows`, `doorsOf`, `standingConsentFor`), `agentDoorsHtml()` in `entityFormPage.ts`, `runDoorCommand` in `entityFormHost.ts` | Doors have one owner each — the form lists the live ones and links to the owning command; nothing live, no footer. **A never-ask entry is one of those doors** (#95, S3.3) and is listed first, with the modal-free ones: `standingConsent` is `ask === 'never'` AND `use === true`, both halves, because the row says an agent may USE this entry without being asked and with `use` off no agent can use it at all. The predicate is `standingConsentFor` in `mcpAccess.ts`, beside the resolver — a broker check or a command needing the same answer must not import a UI module to get it. Its wording says *saved as*, since it is the one row whose value the form itself can change, and it is the one row with **no manage link**: its control is a few lines above it on the same form, and `editNode` there would re-open the form and discard what somebody had typed. The CLI row's claim that the alias routes have **no consent modal was false** and is corrected in the same change: `handleAlias` mints a grant and calls `perform`, which calls `consent` — an alias call raises the same dialog a token call does. **Its replacement was then false the other way** and S4.3 corrected that too: *whether a dialog appears follows this entry's consent setting* handed the alias door to a cadence that cannot reach it, since `preConsent` is wired into `mcpDoor` alone (D1) while `handleAlias` mints a fresh grant per call. The row now says it asks **every time, whatever the consent setting above says** — the one place that matters, because it sits under a row telling the same person an agent may use this entry without being asked. The test reads the route as well as the text: `handleAlias`'s own slice of `credsAgentServer.ts` must not mention `preConsent`, so re-wiring the door turns the sentence red instead of quietly making it false a third time. The code-access key's *no consent modal* is true and stays |
