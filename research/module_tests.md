...
| the contract is regenerated from a FRESH binary | **script guard** | `emit-mcp-tools.mjs` refuses a `creds-mcp` older than its newest `.cs`. `dotnet build dew_flow_creds_for_devs.slnx` does not build `src_mcp` at all, so a `Program.cs` edit plus that command reports success and regenerates the contract from the previous binary — observed exactly that way, mid-story. Watched red by `touch`ing `Program.cs`: *built 0.0h BEFORE Program.cs*. The check is the harness's own, moved to `scripts/mcpBinary.cjs` so both drivers share one copy |
