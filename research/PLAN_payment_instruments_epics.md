...
| S1.4 (cont.) | | 10 plan rounds in the end. Rounds 4–9 chased progressively narrower crash windows and one of MY fixes for them introduced a real bug the next round caught (an in-flight create's node is absent, so the sweep would have purged it). The residual is closed and the cross-window gap is `todo/PLAN_cross_window_write_coordination.md`. Honest verdict: the first three rounds earned their cost; the rest was over-engineering |
