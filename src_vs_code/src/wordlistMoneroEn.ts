/**
 * The Monero English wordlist — 1626 words, exactly as monero-project publishes them.
 *
 * <p>DATA ONLY, beside the ten BIP-39 tables and for the same reason: `wordlists.ts` is the
 * mechanism, this is one of its tables.</p>
 *
 * <p><b>This list was the one deviation the payment plan shipped without</b>, and why it recorded the
 * gap rather than guessing is worth keeping: a single wrong word does not fail to compile and does not
 * fail to save — it silently makes a correct seed read as invalid, or an invalid one read as correct.
 * No reachable npm package carries it as plain data (checked: `monerojs` has none; `mymonero-core-js`
 * and `monero-ts` carry it only compiled into WebAssembly), so it comes from the canonical source
 * itself — `src/mnemonics/english.h` in monero-project/monero — and never from typing.</p>
 *
 * <p><b>How it is checked</b>, in `wordlists.test.ts`, four ways: exactly 1626 words; all distinct;
 * <b>all 1626 three-character prefixes distinct</b>, which is the defining property of a Monero list
 * and the thing any truncation or duplication would break; and two real 25-word seeds published by
 * two independent projects, whose checksums hold only if this table is right AND in order. Its
 * membership was also cross-checked against the WebAssembly build above: every one of the 1626 is in
 * there too.</p>
 *
 * <p>Order is load-bearing: a word's position is what the checksum names.</p>
 */
export const MONERO_EN: readonly string[] = [
