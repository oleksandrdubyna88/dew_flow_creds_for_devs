...
export const HELP_ARTICLES: readonly HelpArticle[] = [
  ...
  },
  {
    ...
    en: {
      ...
      usage: 'Double-click opens the read-only viewer; Edit is on the right-click. New arrivals — created, imported or accepted — are revealed and briefly tinted green so “it worked” and “here it is” are one event. Deleted entries go to the Trash, which can empty itself on a timer.',
    ...
    ru: {
      ...
      usage: 'Двойной клик открывает вьюер; Edit — по правому клику. Новые строки — созданные, импортированные, принятые — показываются и коротко подсвечиваются зелёным: «получилось» и «вот оно» — одно событие. Удалённое уходит в корзину, которая умеет чиститься по таймеру.',
