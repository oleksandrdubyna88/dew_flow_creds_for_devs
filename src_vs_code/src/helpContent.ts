...
export const HELP_ARTICLES: readonly HelpArticle[] = [
  ...
  },
  {
    ...
    en: {
      ...
      whatItIs: 'A payment instrument is one entry in three forms: a **card**, a set of **bank details**, or a **phrase** you must not lose. All three are stored the same way — one encrypted record under one key in the OS keychain — and the form decides which fields you are asked for.\n\n**Two of the three work in this build.** The card and the bank details are complete. The **phrase** form and the option to store a value **woven with a decoy** are built and switched OFF, because the half that reads a woven value back — a payment card in the entry viewer — does not exist yet. A value stored woven today could never be shown again, so nothing here lets you store one. Everything below marked *(not yet)* describes what those switches will do when they turn on.\n\nSwitching an entry to a different form DELETES what the old form held. You are asked first, by field name, and only when something is actually stored.',
      ...
      setup: 'Add an entry in a folder typed to **payment**, or pick the kind on the form. Choose the form, fill in what you have. A card number names its payment system as you type it, and says when the digits do not add up — worth checking, and it saves either way, because people hold cards this build has never heard of.\n\n*(Not yet.)* Any of the number, CVV, PIN, IBAN or account number will be markable **woven with a decoy**, and a phrase will be woven against a generated decoy or against a second real key. The boxes are on the form, disabled, with the reason beside them — visible so the feature is not a surprise when it arrives, and off so nothing can be stored in a form nothing can read.',
      usage: '**What weaving is for, exactly** *(not yet — see above)*. A woven field is stored as your value and a decoy shuffled together under one of twelve methods, and **the method is kept nowhere** — not in this vault, not in a backup, not in the sync. Nobody can unweave it but you, from memory.\n\nIt protects against somebody **reading** an open vault: a shoulder, a screen share, a backup file opened on a laptop. It does **not** protect against somebody who can try every possibility — a CVV is a thousand values, and weaving costs them nothing. An odd-length phrase has twelve methods and an even-length one twenty-four; neither is a defence, it is just arithmetic you should not meet for the first time at save time.\n\n**The CVV, the PIN and an assembled phrase will ask a second time before they appear** *(not yet)*. They will be the only fields in this vault that do — everything else is yours the moment the vault is open. That is a deliberate exception, not an oversight. The rung arrives with the viewer card, since the viewer is the surface it guards; today a card is read by opening it for editing, which asks nothing beyond an unlocked vault.\n\n**A share never carries a CVV or a PIN. An export does.** The export says so out loud, with counts and never values, before it writes the file.',
    ...
    ru: {
      ...
      setup: 'Добавьте запись в папке с типом **payment** или выберите вид в форме. Выберите форму и заполните то, что есть. Номер карты сам называет платёжную систему по мере ввода и говорит, если цифры не сходятся, — стоит проверить, но сохранится в любом случае: у людей есть карты, о которых эта сборка не знает.\n\n*(Пока нет.)* Любое из полей — номер, CVV, PIN, IBAN, номер счёта — можно будет пометить **перемешать с приманкой**, а фразу — перемешать со сгенерированной приманкой или со вторым настоящим ключом. Флажки на форме есть, они отключены, и рядом написано почему: видимые — чтобы функция не свалилась как сюрприз, выключенные — чтобы ничего нельзя было сохранить в виде, который никто не прочитает.',
      usage: '**Для чего именно перемешивание** *(пока нет — см. выше)*. Перемешанное поле хранится как ваше значение и приманка, вплетённые друг в друга одним из двенадцати методов, и **метод не хранится нигде** — ни в сейфе, ни в резервной копии, ни в синхронизации. Расплести это не может никто, кроме вас, по памяти.\n\nЭто защищает от того, кто **читает** открытый сейф: взгляд через плечо, демонстрация экрана, файл резервной копии на ноутбуке. Это **не** защищает от того, кто может перебрать все варианты: CVV — тысяча значений, и перемешивание ему ничего не стоит. У фразы нечётной длины двенадцать методов, у чётной — двадцать четыре; ни то ни другое не защита, это просто арифметика, с которой не стоит впервые встречаться при сохранении.\n\n**CVV, PIN и собранная фраза будут спрашивать второй раз, прежде чем появиться** *(пока нет)*. Это будут единственные поля в сейфе, которые так делают, — всё остальное ваше с момента, когда сейф открыт. Это осознанное исключение, а не недосмотр. Ступенька появится вместе с карточкой просмотра, потому что именно её она и охраняет; сегодня карту читают, открыв её на редактирование, а это не спрашивает ничего сверх открытого сейфа.\n\n**Шаринг никогда не несёт CVV и PIN. Экспорт — несёт.** Экспорт говорит об этом прямо, числами и никогда значениями, до того как напишет файл.',
