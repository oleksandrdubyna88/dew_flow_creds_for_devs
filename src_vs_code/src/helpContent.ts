...
export const HELP_ARTICLES: readonly HelpArticle[] = [
  ...
  },
  {
    ...
    en: {
      ...
      setup: 'In this order, and the Windows step has two routes that end differently. **On Windows**, keep a VS Code window open with the vault unlocked, then either: Install… → *Copy install command for another machine…* → `creds-mcp` → Windows, and run it in PowerShell here — it installs into `%LOCALAPPDATA%\\Programs\\creds` and puts that on your PATH, which WSL inherits, so nothing else is needed; or Install… → *Install the MCP Server…*, which puts the binary in the extension’s own storage, deliberately NOT on the PATH — then you must name it later. **Inside the distribution:** Install… → *Copy install command for another machine…* → `creds-mcp` → “Linux, WSL or a container”, paste it into your WSL shell (it verifies the download and installs to `~/.local/bin`), then register it: `claude mcp add -s user creds -- ~/.local/bin/creds-mcp` — adding `-e CREDS_MCP_WINDOWS_BINARY=/mnt/c/...` only if you took the second Windows route.',
    ...
    ru: {
      ...
      setup: 'Строго в таком порядке, и у шага на Windows два маршрута с разным следствием. **На Windows** держите открытым окно VS Code с разблокированным сейфом, дальше либо: Install… → *Copy install command for another machine…* → `creds-mcp` → Windows, и выполните это в PowerShell здесь — бинарь ляжет в `%LOCALAPPDATA%\\Programs\\creds` и допишется в PATH, который WSL наследует, и больше ничего не нужно; либо Install… → *Install the MCP Server…*, которая кладёт бинарь в хранилище расширения и намеренно НЕ на PATH — тогда его придётся назвать явно. **Внутри дистрибутива:** Install… → *Copy install command for another machine…* → `creds-mcp` → «Linux, WSL or a container», вставьте в шелл WSL (скрипт проверит контрольную сумму и поставит в `~/.local/bin`), затем зарегистрируйте: `claude mcp add -s user creds -- ~/.local/bin/creds-mcp` — с `-e CREDS_MCP_WINDOWS_BINARY=/mnt/c/...` только если выбрали второй маршрут на Windows.',
