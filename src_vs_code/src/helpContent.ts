...
export const HELP_ARTICLES: readonly HelpArticle[] = [
  ...
  },
  {
    ...
    en: {
      ...
      usage: 'The tree marks an opened entry with a pentagon — each lit edge one switch of the entry ladder. Rotation happens through a `{{creds:new}}` placeholder, so no value ever enters the agent’s context. An agent sees folders through `creds_folders` and changes them with `creds_create_folder` / `creds_edit_folder` / `creds_delete_folder`. Every action still raises the consent modal.',
    ...
    ru: {
      ...
      usage: 'Открытая запись помечена в дереве пятиугольником — каждая горящая грань один переключатель записевой лестницы. Ротация идёт через плейсхолдер `{{creds:new}}`: значение никогда не попадает в контекст агента. Папки агент видит через `creds_folders`, а меняет через `creds_create_folder` / `creds_edit_folder` / `creds_delete_folder`. Каждое действие всё равно поднимает модал согласия.',
