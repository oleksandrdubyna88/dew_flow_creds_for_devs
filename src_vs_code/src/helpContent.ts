...
export const HELP_ARTICLES: readonly HelpArticle[] = [
  ...
  },
  {
    ...
    mediaSlots: [],
    en: {
      title: 'A PIN on one entry, or on everything in a folder',
      whatItIs: 'Right-click an entry and choose **Protect with a PIN…**. Every secret it holds — the password, the private key, the notes, the login and URL, the payment details, the config body, the connection string, the VPN configuration, the one-time-code seed — is re-encrypted under a PIN of your own, on top of the encryption the vault already gives it.\n\nOn a folder, **Protect Every Entry with a PIN…** does exactly that to every entry inside, at any depth. It does not encrypt the folder: a folder is a place, and the protection belongs to the entries.\n\n**Remove PIN Protection…** takes it back off, given the PIN.',
      why: 'The vault is one lock. Everything inside it opens together, which is what makes it usable — and it means an open vault is a readable vault. A PIN on an entry is the second lock, for the few things that deserve one: the production database, the payment card, the key that signs releases.\n\nIt is a real re-encryption, not a prompt in front of a value that is already readable. A fresh random key seals the value; the PIN seals that key; the plaintext is nowhere. Nothing in this build can read it without the PIN, which is the point and also the cost.',
      setup: 'Right-click → **Protect with a PIN…**, then type the PIN twice. It is stored NOWHERE — not here, not in a backup, not in the sync — so the second box is the only check there will ever be.\n\nOn a folder, entries that already have a PIN of their own are **skipped and named before the run starts**: you are told how many, and that they keep the PIN they have. A folder can legitimately hold entries under two different PINs, and this tells you when you are about to create that rather than letting you discover it later. When the folder already holds protected entries, the PIN you type is CHECKED against them and you are told how many it opens — *"this PIN opens 4 of the 7 protected entries here"* — before anything is written.',
      usage: '**Sharing one, and receiving one.** Sharing a protected entry asks you for its PIN — the values have to be unwrapped before they can travel, because the person receiving them does not have your PIN and never will. So the copy they get is NOT protected by yours: they are asked to choose one of their own when they accept it, and declining imports nothing. Their PIN is theirs; yours never leaves this machine.\n\n**A folder can go on asking after you empty it.** Running **Protect Every Entry with a PIN…** also sets the folder to ask for a PIN on every entry created in it afterwards — including when the folder was empty at the time, which is the one case nothing else could tell. Right-click the folder for **Stop Asking for a PIN Here** to turn that back off; it changes nothing about the entries, which keep their own PINs.\n\n**Opening a protected entry asks once.** Type the PIN and the whole entry opens; every field afterwards uses what you typed, for as long as this window lives. Closing the window, reloading it, or locking the vault forgets it. Nothing is written down.\n\n**Agents do not see a protected entry at all.** Not "refused when used" — absent from the listing, absent from the search, and a direct request by id is answered the way a made-up id is answered. An agent that can see an entry it can never open will keep asking, and every one of those is a door you have to answer.\n\n**Nothing automatic uses it.** An environment variable, a terminal, the SSH broker, a `creds://` reference and an agent rotation each say why instead of guessing. A rotation is refused rather than silently unmarked: it would store a new readable value while the entry went on claiming to be protected.\n\n**The health report skips it**, and says so. Ciphertext always looks like a strong unique password, so scanning it would tell you your weakest habit is fine.\n\n**A backup keeps the protection.** What is exported is what is stored — still wrapped — so a restored entry still needs its PIN.',
      whatCanGoWrong: '**A forgotten PIN is gone data.** There is no recovery: not by us, not from a backup, not from the sync. The vault recovery code opens the VAULT; it does not open an entry. Read the second box before you type into it.\n\n**A folder run protects the entries, not the folder.** New entries added afterwards are not protected by the fact that their neighbours are — protect them, or run the folder again.\n\n**Interrupted, a run leaves a mix.** Some entries protected, some not, and every one of them readable by its own means. There is nothing to repair and nothing to resume: run it again and it finishes, skipping what is already done.',
    },
    ru: {
      title: 'ПИН на одной записи или на всём содержимом папки',
      whatItIs: 'Правый клик по записи → **Protect with a PIN…**. Каждый секрет, который она хранит — пароль, приватный ключ, заметки, логин и URL, платёжные данные, тело конфига, строка подключения, конфигурация VPN, seed одноразовых кодов — перешифровывается вашим ПИНом поверх того шифрования, которое хранилище даёт ему и так.\n\nНа папке **Protect Every Entry with a PIN…** делает ровно это со всеми записями внутри, на любой глубине. Папку это не шифрует: папка — место, а защита принадлежит записям.\n\n**Remove PIN Protection…** снимает защиту, если ввести ПИН.',
      why: 'Хранилище — один замок. Всё внутри открывается вместе, и именно поэтому им удобно пользоваться — и именно поэтому открытое хранилище читаемо. ПИН на записи — второй замок для тех немногих вещей, которые его заслуживают: боевая база, платёжная карта, ключ, которым подписываются релизы.\n\nЭто настоящее перешифрование, а не запрос перед уже читаемым значением. Случайный ключ запечатывает значение, ПИН запечатывает этот ключ, открытого текста нет нигде. Без ПИНа сборка прочитать его не может — в этом и смысл, и цена.',
      setup: 'Правый клик → **Protect with a PIN…**, затем ввести ПИН дважды. Он не хранится НИГДЕ — ни здесь, ни в копии, ни в синхронизации — поэтому второе поле и есть единственная проверка, которая когда-либо будет.\n\nНа папке записи, у которых уже есть свой ПИН, **пропускаются и называются до запуска**: вам говорят, сколько их и что они сохранят свой ПИН. В папке законно могут лежать записи под двумя разными ПИНами, и это сообщается заранее, а не обнаруживается потом. Если в папке уже есть защищённые записи, введённый ПИН ПРОВЕРЯЕТСЯ по ним, и вам говорят, сколько он открывает — «этот ПИН открывает 4 из 7 защищённых записей» — до того, как что-то будет записано.',
      usage: '**Шаринг и приём.** Шаринг защищённой записи спрашивает у вас её ПИН: значения нужно развернуть перед отправкой, потому что у получателя вашего ПИНа нет и не будет. Поэтому копия у него вашим ПИНом НЕ защищена — при приёме ему предлагается выбрать свой, а отказ не импортирует ничего. Его ПИН — его; ваш не покидает эту машину.\n\n**Папка может продолжать спрашивать, даже когда вы её опустошили.** Команда **Protect Every Entry with a PIN…** заодно ставит папке признак «спрашивать ПИН у каждой создаваемой здесь записи» — в том числе если папка была пуста, а это единственный случай, о котором иначе узнать неоткуда. Правый клик по папке → **Stop Asking for a PIN Here**, чтобы это выключить; записей это не трогает, свои ПИНы они сохраняют.\n\n**Открытие защищённой записи спрашивает один раз.** Ввели ПИН — открылась вся запись; все поля дальше используют введённое, пока живёт это окно. Закрытие окна, перезагрузка и блокировка хранилища его забывают. Никуда он не записывается.\n\n**Агенты защищённую запись не видят вообще.** Не «отказ при использовании» — её нет в списке, нет в поиске, а прямой запрос по id получает тот же ответ, что и выдуманный id. Агент, который видит запись и не может её открыть, будет спрашивать снова, и на каждый такой запрос кому-то придётся отвечать.\n\n**Автоматика ею не пользуется.** Переменная окружения, терминал, SSH-брокер, ссылка creds:// и ротация агентом объясняют почему, а не гадают. Ротация отклоняется, а не снимает метку молча: она записала бы новое читаемое значение, пока запись продолжает называть себя защищённой.\n\n**Отчёт о здоровье её пропускает** и говорит об этом. Шифротекст всегда выглядит сильным и уникальным паролем, так что проверка сказала бы вам, что худшая ваша привычка — в порядке.\n\n**Резервная копия сохраняет защиту.** Экспортируется то, что хранится — по-прежнему завёрнутое, — так что восстановленной записи ПИН всё ещё нужен.',
      whatCanGoWrong: '**Забытый ПИН — потерянные данные.** Восстановления нет: ни у нас, ни из копии, ни из синхронизации. Код восстановления открывает ХРАНИЛИЩЕ, но не запись. Прочитайте текст во втором поле, прежде чем в него печатать.\n\n**Прогон по папке защищает записи, а не папку.** Добавленные потом записи не становятся защищёнными оттого, что защищены соседние — защитите их или прогоните папку снова.\n\n**Прерванный прогон оставляет смесь.** Часть записей защищена, часть нет, и каждая читается своим способом. Чинить нечего и возобновлять нечего: запустите снова, и прогон закончит работу, пропустив уже сделанное.',
    },
  },
  {
    id: 'woven-password',
    mediaSlots: [],
    en: {
      title: 'A password stored woven with a decoy',
      whatItIs: 'The **General** section of a credential has *Store this password woven with a decoy*. Tick it, pick one of the twelve methods, and what goes into the keychain is your password and a made-up one of the same shape, interleaved character by character.\n\nThe methods are the same twelve a card’s PIN uses, offered in an order drawn afresh every time — but “Method 5” names one algorithm everywhere: in the form where you weave a value, and in the viewer where you read it back.',
      why: 'A vault that is open is a vault that can be read — over a shoulder, in a screen share, in a backup file opened on a laptop. A woven password is the one value in it that is still not readable, because what separates it from its decoy is a choice in your memory and is written down nowhere: not here, not in a backup, not in the sync.\n\nWhat it does NOT buy is protection from somebody who can try every possibility. Twelve methods is twelve tries. This is against reading, not against guessing.',
      setup: 'Open the credential, tick the box in **General**, choose a method from the picker that appears, and type the password. The entry then says **Woven — on**, on the form and in the viewer.\n\nToo short to weave — weaving needs at least two characters — or no method chosen, and the form ASKS before it saves, rather than quietly storing the password in the clear.',
      usage: '**Reading it back.** The viewer shows a method picker and two rows of characters. Pick the method you chose and read the row you recognise; each row has its own Copy button. A wrong method answers in exactly the same shape as a right one — no tick, no “looks valid”, no ordering that puts a likelier answer first. That is deliberate: anything that could tell them apart would do the guessing for whoever is reading over your shoulder.\n\n**The decoy has the same length and the same character sets** as the real password. A character from a set your password does not use would be provably decoy, and the halves would separate by inspection.\n\n**Nothing automatic will use it.** An environment variable, a terminal, the SSH broker, a creds:// reference and an agent rotation are each handed a sentence saying why, rather than a guess — a wrong password injected into a login is an account lockout nobody watches happen. And withheld is said as withheld: none of them reports “nothing stored” about a password that is very much stored.\n\n**Replacing it keeps it woven.** The box arrives already ticked for an entry that has it, so a pasted replacement stays protected. Untick it deliberately to store the new password in the clear.\n\n**A share carries the mark**, so whoever you shared with is told the value is woven instead of opening an entry full of gibberish with nothing explaining it.',
      whatCanGoWrong: '**A forgotten method is a lost password.** There is no recovery — not by us, not from a backup, not from the sync — because the original is in none of them. Twelve is a small enough number to work through by hand if you must, and that is the honest scale of what this protects against.\n\n**The stored value cannot be unwoven.** That needs the method, and nothing in this build has it. What you can always do is REPLACE the password, which is a different act and is what the form offers.\n\n**An agent cannot rotate it.** A rotation would store a new, unwoven value while the entry went on claiming to be woven, so it is refused rather than silently unmarked — unmarking is a decision about your protection. Rotate it from the entry instead.\n\n**Clearing the password clears the mark with it.** An entry with nothing stored does not go on offering two rows to read.',
    },
    ru: {
      title: 'Пароль, перемешанный с приманкой',
      whatItIs: 'В секции **General** у credential есть галочка *Store this password woven with a decoy*. Поставьте её, выберите один из двенадцати методов — и в keychain уйдёт ваш пароль вперемешку с выдуманным той же формы, символ за символом.\n\nМетоды те же двенадцать, что и у PIN карты, и порядок в списке каждый раз новый — но «Метод 5» везде означает один и тот же алгоритм: и в форме, где вы перемешиваете, и в просмотре, где читаете обратно.',
      why: 'Открытое хранилище — это хранилище, которое можно прочитать: через плечо, в демонстрации экрана, в резервной копии, открытой на ноутбуке. Перемешанный пароль — единственное значение, которое и там остаётся нечитаемым, потому что отделяет его от приманки только ваш выбор в памяти, и он не записан нигде: ни здесь, ни в копии, ни в синхронизации.\n\nЧего это НЕ даёт — защиты от того, кто может перебрать всё. Двенадцать методов — двенадцать попыток. Это против чтения, а не против перебора.',
      setup: 'Откройте запись, поставьте галочку в **General**, выберите метод в появившемся списке и введите пароль. После этого запись пишет **Woven — on** и в форме, и в просмотре.\n\nСлишком короткий пароль (перемешивание требует минимум двух символов) или невыбранный метод — и форма СПРОСИТ перед сохранением, а не сохранит пароль открытым молча.',
      usage: '**Как прочитать обратно.** В просмотре появляется выбор метода и две строки символов. Выберите свой метод и прочитайте ту строку, которую узнаёте; у каждой своя кнопка копирования. Неверный метод отвечает ровно в той же форме, что и верный — ни галочки, ни «похоже на правду», ни порядка, ставящего вероятный ответ первым. Это намеренно: всё, что различало бы их, угадывало бы за того, кто смотрит вам через плечо.\n\n**У приманки та же длина и те же наборы символов**, что и у настоящего пароля. Символ из набора, которого в вашем пароле нет, был бы доказуемо приманкой.\n\n**Автоматика им не пользуется.** Переменная окружения, терминал, SSH-брокер, ссылка creds:// и ротация агентом получают фразу с объяснением, а не догадку: неверный пароль, подставленный в логин, — это блокировка аккаунта, которую никто не увидит. И отказ произносится как отказ: никто из них не говорит «ничего не сохранено» о пароле, который сохранён.\n\n**Замена сохраняет перемешивание.** У записи, где оно включено, галочка приходит уже отмеченной, так что вставленный новый пароль тоже уйдёт перемешанным. Снимите её осознанно, чтобы сохранить замену открытой.\n\n**В шаринг метка едет вместе со значением**, иначе получатель откроет запись с нечитаемой строкой и без единого объяснения на экране.',
      whatCanGoWrong: '**Забытый метод — потерянный пароль.** Восстановления нет: ни у нас, ни из копии, ни из синхронизации — оригинала нет ни в одной из них. Двенадцать — число, которое при необходимости перебирается руками, и это честный масштаб того, от чего защита.\n\n**Сохранённое значение расплести нельзя.** Для этого нужен метод, а его в сборке нет. Что можно всегда — ЗАМЕНИТЬ пароль; это другое действие, и именно его предлагает форма.\n\n**Агент не может его ротировать.** Ротация сохранила бы новое, неперемешанное значение, пока запись продолжала бы называть себя перемешанной — поэтому она отклоняется, а не снимает метку молча: снятие метки — решение о вашей защите. Ротируйте из записи.\n\n**Очистка пароля снимает и метку.** Запись, где ничего не хранится, не предлагает читать две строки.',
    },
  },
  {
    ...
    ru: {
      ...
      whatItIs: 'Платёжный инструмент — одна запись в трёх формах: **карта**, **банковские реквизиты** или **фраза**, которую нельзя потерять. Все три хранятся одинаково — одна зашифрованная запись под одним ключом в хранилище ОС, — а форма решает, какие поля у вас спросят.\n\n**Работают все три.** Карта и реквизиты готовы полностью, любое их поле можно хранить **перемешанным с приманкой**, а фраза вводится в форму, получает свой словарь и хранится перемешанной со второй колонкой — сгенерированной приманкой или вашими собственными словами, если нужно, чтобы одна запись держала два настоящих ключа. Раскладка «рядом» предлагается только при чётном числе слов и говорит почему, когда не предлагается.\n\nПереключение записи на другую форму УДАЛЯЕТ то, что хранила прежняя. Спрашивают заранее, по именам полей и только если там действительно что-то есть.',
