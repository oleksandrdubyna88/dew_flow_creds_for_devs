...
/** The most rows one tab holds. Past it the oldest are dropped, and the page says so. */
...
export class EventTab {
  ...
  constructor(
    private readonly client: OrgEventsClient,
  ...
  private trim(): void {
    ...
    this.rows = this.rows.slice(-MAX_ROWS_IN_TAB);
  }
