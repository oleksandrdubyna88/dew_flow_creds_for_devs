...
- **Edit → General → Not for export.** A marked entry is not offered *Share with…* or *Export / Share
  Externally…*, and both refuse it when it is part of a multi-selection. Sharing or
  ...
  its one-time code is not asked about. An accepted update of a shared entry keeps the recipient's
  own mark. *Share with Claude Code…*, backup, sync and local use are
...
- **The weaving controls are reachable from the bank form, and the save no longer weaves behind
  your back.** The method picker, the warning and the picture were emitted inside the Card
  fieldset, which is hidden whenever the form is not a card. Ticking *Store the IBAN woven with a
  decoy* on a bank form therefore showed nothing at all — and the save wove it anyway, reading the
  method off the hidden picker. The method is stored nowhere, so an IBAN woven that way was
  unreadable from the moment it was written, by anyone, forever. The shared controls now sit after
  both fieldsets and appear as soon as any weave box is ticked, wherever that box lives.

- **The password example is drawn in colour, inside its box.** Every colour rule on the form is
  scoped to the example block, the card form created one, and the password form appended the same
  three columns into a plain container — so what should show your value in green, the decoy in
