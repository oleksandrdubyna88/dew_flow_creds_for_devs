...
// WHO is asking, as a client REPORTS it: an optional object in every body that performs
// something, so the consent modal can name the agent, its session and its folder instead of
// saying "Claude Code" to everyone. It is a LABEL — the window strips and caps it and decides
// nothing with it — which is why it may travel in an unauthenticated body at all. Both shapes are
// declared: the nested object under `field` is what the senders use (measured under AOT, plan
// §5.2), and the flat `<flatPrefix><Field>` fields are the fallback the window also reads. Read
// off the module rather than retyped here, so a field added there cannot be missing here.
const caller = {
  field: callerModule.CALLER_FIELD,
  fields: [...callerModule.CALLER_FIELDS],
  flatPrefix: callerModule.CALLER_FLAT_PREFIX,
  maxFieldChars: callerModule.CALLER_MAX_FIELD_CHARS,
  maxLabelChars: callerModule.CALLER_MAX_LABEL_CHARS,
};
